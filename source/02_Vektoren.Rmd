...
- Wenn einer Funktion mehrere Argumente übergeben werden, werden diese
  mit Kommata separiert, etwa: `c(1, 2, 3)`[^kommasep]
- Funktionen führen eine Berechnung durch und geben uns das Ergebnis
  zurück

[^kommasep]: In den obigen Beispielen einfacher statistischer
...
solche Operationen im Allgemeinen **paarweise**[^komponentenweise]
angewendet:

[^komponentenweise]: Ich werde dazu oft auch *komponentenweise* sagen.
...
"gepaart".[^ganzodereins]

[^ganzodereins]: Wir werden nur diese Fälle betrachten: Entweder wird
...
[^NA]: Eigentlich ist `NA` kein eigener Datentyp. In `R` hat
...
*optionalen Argument* `na.rm`[^optional] der Funktion `mean`,
...
`pnorm` herausfinden:[^pnorm]

[^pnorm]: `pnorm` ist die kummulative Verteilungsfunktion der
