...
Spalten können in `R` mit Namen addressiert werden. Und: In solchen
Tabellen wird gelten, dass jedes Item in genau einer Spalte steht. Es
ist beispielsweise vorstellbar, dass wir mit dem folgenden Befehl die
...
`factor` umzuwandeln. Dabei kann man die `levels` spezifizieren, d.h.
die Werte, die der Vektor annimmt, **bevor** er in `factor` umgewandelt
wird -- hier `1`, `2` und `3`. Das Argument `labels` wird verwendet, um
...
Um zu testen, ob ein oder mehrere Element in einem Vektor enthalten
