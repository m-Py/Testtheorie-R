...
```{block, type = "block"}
**Merke**: In `R` kann (fast) alles in einer Variablen gespeichert und weiter verwendet werden.
```
...
durchnummerierte Itemnamen als `character`-Vektor generieren:
...
```{r}
paste0("Extraversion", 1:5)
```

Mit der Funktion `mode` können wir überprüfen, welchen Datentyp ein
Vektor hat, also ob er beispielsweise tatsächlich vom Typ `character`
ist:[^`r incref()`]
