...
### Logische Operationen mit `NA`

Oftmals will man herausfinden, ob in Vektoren fehlende Werte vorliegen (also ob 
einige Elemente darin `NA` sind). Wir können auch mit der Funktion `is.na()` 
eine logische Abfrage durchführen, die für jedes Element in einem überprüft, ob 
es `NA` ist. 
...
Die Funktion `is.na()` prüft in einem Vektor, welche Werte `NA` sind und gibt an 
den entsprechenden Positionen `TRUE` zurück, ansonsten `FALSE`. Die Ausgabe von 
`is.na()` ist also ein logischer Vektor, der genauso lang ist wie der 
Eingabevektor. 
