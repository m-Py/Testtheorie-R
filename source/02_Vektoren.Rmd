...
In erster Linie werden wir Vektoren vom Typ `character` für Datenzugriffe
...
dasselbe wie im Beispiel `meinVektor[auswahl]` oben, nur, dass ich dort
...
Durch Klammerung können wir die *Präzedenz* von `R`-Befehlen steuern. Präzedenz 
bezieht sich auf die Reihenfolge, in der `R`-Befehle ausgeführt werden. 
`R`-Befehle werden, wie es auch bei normalen mathematischen Berechnungen der 
Fall ist (siehe [Punkt vor 
Strich](https://de.wikipedia.org/wiki/Punktrechnung_vor_Strichrechnung)), nicht 
unbedingt einfach von links nach rechts durchgeführt. 

Folgender Code zeigt, dass logische Vergleiche wie `>` oder `<` eine höhere 
Präzedenz haben als das logische ODER:
...
In diesem Fall werden die Unterbefehle `daten < 3` und `daten > 6` separat 
evaluiert, und die beiden Ergebnisse werden danach mit dem logischen ODER 
verknüpft. Es wird nicht das Ergebnis von `daten < 3` per ODER mit `daten` 
verknüpft, und erst im Anschluss die `> 6`-Operation durchgeführt, was bei einer 
reinen Durchführung von rechts nach links der Fall wäre. Das liegt übrigens 
nicht daran, dass diese Durchführung auch sinnlos wäre -- das weiß `R` im 
Zweifel nicht -- `R` geht rein nach Präzedenz der Operatoren vor. 

Um sicherzugehen, sieht man in diesem Fall oft folgenden Code, der durch 
...
Diese Klammerung ist jedoch nicht nötig. 

Betrachten wir noch das folgende weiterführende Beispiel: 
...
Durch Klammerung oder durch Zwischenspeichern in einer Variablen können wir das 
