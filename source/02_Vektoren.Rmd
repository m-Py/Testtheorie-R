...
vielen Elementen besteht wie der Eingabevektor. 

Auch basale mathematische Berechnungen werden gleich auf alle Elemente 
...
einmal vernachlässigen; interessierte Leser können die Befehle `c(1,2) *
1:4` und `c(1,2) * 1:3` in die `R`-Konsole eingeben und beobachten, was
passiert.
...
zusammenfügen kann -- im Allgemeinen beliebig viele.
...
Die logischen Operationen UND und ODER verknüpfen zwei logische Elemente
miteinander (logische Elemente = `TRUE`/`FALSE`). Die ODER-Operation
ergibt `TRUE`, sobald mindestens eines der Elemente `TRUE` ist. Das
logische UND ergibt nur dann `TRUE`, wenn beide Elemente `TRUE` sind.
...
Beachtet, dass sich ein Faktor in den meisten Fällen genauso wie ein 
Vektor vom Typ `character` verhält. Inhaltlich macht das Sinn: In beiden 
Fällen ist ein Vektorelement ein „Text“ beziehungsweise eine verbale 
Beschreibung. Der Unterschied ist, dass wir Faktoren oftmals so 
generieren, dass wir eine numerische Kodierung in eine für Menschen 
verständliche verbale Beschreibung umwandeln. Bei einer Dateneingabe 
werden oftmals numerische Kodierungen eingegeben, da dies schneller geht 
als einen Text einzugeben.[^Textfeld] Ein Beispiel dafür, dass sich 
Faktoren genauso verhalten wie Vektoren vom Typ `character`, findet sich 
im Abschnitt [Logische Vergleiche](#logischevergleiche) in diesem 
Kapitel.

[^Textfeld]: Das macht natürlich nur dann Sinn, wenn es eine 
vordefinierte Anzahl an Ausprägungen einer Variable gibt, nicht etwa 
wenn Studienteilnehmer*innen ein freies Textfeld ausfüllen. 
...
benennen kann (per `na.rm = `), was wir aber nicht immer machen. Dazu 
mehr in [Kapitel 3](#subset).
...
nachzudenken, warum wir die Funktion `mean` hier verwenden können, um
einen relativen Anteil zu bestimmen. Normalerweise sind wir es eher
gewohnt, dass Mittelwerte und relative Anteile etwas Unterschiedliches
sind. Der Grund dafür ist folgender: Für eine Variable, bei der die
Werte nur 1 oder 0 annehmen können (analog in R: `TRUE`/`FALSE`),
entspricht der Mittelwert dieser Variablen dem relativen Anteil der
Werte, die 1 bzw. `TRUE` sind. Als Beispiel betrachten wir den folgenden
Vektor:
