...
solche Operationen im Allgemeinen **komponentenweise**:
...
[^linkbool]:
[https://de.wikipedia.org/wiki/Boolesche_Algebra#Zweielementige_boolesche_Algebra](https://de.wikipedia.org/wiki/Boolesche_Algebra#Zweielementige_boolesche_Algebra)
...
ODER-Operation**. Wenn UND und ODER in einem logischen Ausdruck verbunden
werden, wird zunächst die UND und dann die ODER-Operation ausgeführt,
unabhängig davon, in welcher Reihenfolge wir die Befehle aufschreiben.
Möchten wir forcieren, dass die ODER-Operation zuerst durchgeführt wird,
können wir -- ganz analog zu mathematischen Berechnungen -- Klammern
verwenden:
...
[^kommtspaeter]: Das Beispiel mag hier artifiziell wirken, aber genau 
so etwas werden wir in [Kapitel 4](#trennschaerfe) machen.

```{r}
1:10[-8]
```

Das hat aber nicht funktioniert, die 8 ist in der Ausgabe enthalten.
...
```{block, type = "block"}
**Merke**: Im Zweifel verwenden wir lieber einmal zu viel als einmal zu 
wenig Klammern.
```
