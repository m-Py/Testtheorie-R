...
Taschenrechner-Berechnungen oben. So gilt für die Berechnung `1 + 3`:
...
Element, sondern gleich eine Aneinanderreihung beliebig vieler
gleichartiger Elemente, etwa Zahlen.[^`r incref()`] Statistische
Berechnungen -- wie die Berechnung eines Mittelwerts oder einer
Standardabweichung -- lassen sich direkt auf einer Menge an Daten
durchführen, da diese in **einem** Vektor gespeichert sind. Diese
"Vektorbasiertheit" ist vermutlich die größte Stärke von `R` für
statistische Berechnungen.

[^`r incfnc()`]: Interessanterweise gibt es sogar Vektoren der Länge 0
-- also Vektoren, die gar kein Element beinhalten. Das soll uns aber
erst einmal nicht beschäftigen.
...
[^`r incfnc()`]: `R` würde oft auch bei einelementigen Vektoren ein
Ergebnis ausgeben, aber das ist zum Beispiel beim Mittelwert wenig
sinnvoll.
...
- Wenn einer Funktion mehrere Argumente übergeben werden, werden diese
  mit Kommata separiert, etwa: `c(1, 2, 3)`[^`r incref()`]
- Funktionen führen eine Berechnung durch und geben uns das Ergebnis
  zurück

[^`r incfnc()`]: In den obigen Beispielen einfacher statistischer
Berechnungen wird jeweils genau ein Argument übergeben, nämlich der
Vektor, für den wir den Mittelwert, die Standardabweichung
etc. berechnen wollten. Es ist auch möglich -- und auch üblich --, dass
Funktionen mehrere Argumente annehmen, die ihr Verhalten bestimmen. Die
Funktion `plot` etwa verfügt über eine kaum überschaubare Menge an
möglichen Argumenten, die verwendet werden können, um das Aussehen einer
Abbildung zu spezifizieren.
...
`sqrt` und `round` geben hingegen einen Vektor zurück, der aus genauso
viele Elementen besteht wie der Eingabevektor. Auch basale mathematische
Berechnungen werden gleich auf alle Elemente eines Vektors angewendet:
...
der Vektoren `1:10` bzw. `1:10 * 2` angewendet; die Ausgabe ist demnach
jeweils ein Vektor der Länge 10. Bei gleich langen Vektoren werden
solche Operationen im Allgemeinen **paarweise**[^`r incref()`]
angewendet:

[^`r incfnc()`]: Ich werde dazu oft auch *komponentenweise* sagen.
...
[^`r incfnc()`]: Wir werden nur diese Fälle betrachten: Entweder wird
ein ein-elementiger Vektor mit einem längeren Vektor verknüpft oder zwei
gleich lange Vektoren werden miteinander verknüpft. Es ist auch möglich,
andere Kombinationen von Vektorlängen zu paaren, was wir jedoch erst
einmal vernachlässigen (gebt bei Interesse einmal die Befehle
`c(1,2) * 1:4` und `c(1,2) * 1:3` in die `R`-Konsole ein).
...
Wir können auch mit "`=`" Daten zu Variablen zuweisen. Das funktioniert
genauso wie "`<-`":
...
vielen anderen Programmiersprachen werden mit `=` Variablen zugewiesen.

### Ausgabe versus Abspeichern {#ausgabevsabspeichern}

Wir haben bereits zwei verschiedene Möglichkeiten gesehen,
Objekte[^`r incref()`] in `R` zu verwenden:

1. Wir geben Objekte in der Konsole aus.
2. Wir speichern Objekte in einer Variable ab.

[^`r incfnc()`]: Bis jetzt kennen wir nur das Vektor-Objekt. In `R` gibt
es aber ganz verschiedene "Datencontainer", die man allgemein als
Objekte bezeichnet.
...
Die Funktion `sort` sortiert den numerischen Vektor `bar`. Wie sieht
der Vektor `bar` nach der Operation aus? Es gibt zwei Möglichkeiten:
...
Offensichtlich hat `sort(bar)` den Vektor `bar` nicht geändert. Das ist
eine fundamentale Eigenschaft von `R`. **Funktionen nehmen Daten an und
sie geben Daten zurück -- sie verändern aber nicht die eingegebenen
Daten**. Wenn wir wollen, dass `bar` die Zahlenfolge in sortierter
Reihenfolge enthält, können wir die folgende Befehlkette verwenden:
...
Generell bestehen Variablennamen in `R` aus Buchstaben und Zahlen und
den Zeichen `.` und `_`. Folgende Einschränkungen sind zu beachten; wenn
diese Anforderungen nicht berücksichtigt werden, wird `R` eine
Variablenzuweisung nicht akzeptieren.
...
entscheidet.[^`r incref()`]

[^`r incfnc()`]: Ich werde von dieser Regel in diesem Skript abweichen.
...
In `R` hat jeder Vektor einen Datentyp. Bis jetzt haben wir nur mit
Zahlen gearbeitet. Dieser Datentyp heißt in `R` "numeric". Der Datentyp
eines Vektors bestimmt, was für Operationen wir damit durchführen
können. Vektoren vom Typ "numeric" etwa kann man addieren,
multiplizieren und so weiter. Es gibt weitere Datentypen, die wir
benutzen, um unterschiedliche Informationen darzustellen.

### `character`

`character` ist der Datentyp, der Text kennzeichnet. Text wird mit
doppelten oder einfachen Anführungszeichen angegeben:
...
etwa ergibt Folgendes eine Fehlermeldung[^`r incref()`] und ergibt auch
gar keinen Sinn, da man Text nicht mit einer Zahl multiplizieren kann:

[^`r incfnc()`]: Leider sind Fehlermeldungen in `R` oftmals sehr
...
Funktion, die Vektoren vom Typ `character` generiert, sei hier jedoch
kurz vorgestellt, da wir von ihr Gebrauch machen werden. Die Funktion
`paste0` kann verwendet werden, um mehrere Vektoren als Text
zusammenzufügen. Das wird nützlich sein, wenn wir in Datentabellen auf
bestimmte Spalten zugreifen wollen, denn dort gilt: Jedes Item steht in
einer Spalte. So lassen sich beispielsweise bequem 10 durchnummerierte
Itemnamen als `character`-Vektor zusammenfügen:
...
Ergebnis ist ein 10-elementiger Vektor, wie wir auch so überprüfen
können:
...
Mit der Funktion `mode` können wir überprüfen, dass der Vektor
tatsächlich vom Typ `character` ist:[^`r incref()`] 

[^`r incfnc()`]: Die Ausgabe des Vektors macht uns auch schon eindeutig
klar, dass es sich hier um einen Vektor vom Typ `character` handelt; die
ausgegeben Elemente erscheinen nämlich in Anführungszeichen ("item_1",
"item_2", ...)
...
miteinander verknüpft, wird immer ein ein-elementiger Vektor vom Typ
...
`paste0` Antworten auf gewünschte Items auswählen, da wir mit
...
Wir werden häufig vom Typ `logical` Gebrauch machen, wenn wir in
...
), ODER (in `R`: `|` ) und NICHT (in `R`: `!` )
durchführen:[^`r incref()`]
...
**Merke**: Auch diese logischen Operationen arbeiten komponentenweise
auf Vektoren, die mehr als ein Element enthalten:
...
`factor` Vektoren stellen kategoriale Variablen dar -- etwa die
unabhängigen Variablen in einer ANOVA. So können wir einen Vektor vom
Typ `factor` erstellen:
...
Die Funktion `factor` kann genutzt werden, um numerische Werte in
`factor` umzuwandeln. Dabei kann man die `levels` spezifizieren,
d.h. die Werte, die der Vektor annimmt, **bevor** er in `factor`
umgewandelt wird -- hier `1`, `2` und `3`. `labels` wird verwendet, um
anzugeben, wie die Faktorstufen angezeigt werden sollen. Das ist ähnlich
den Wertelabels, die man in SPSS vergeben kann. Der Unterschied in `R`:
Wenn ich eine Variable in `factor` umwandle, kann ich damit keine
numerischen Berechnungen mehr durchführen. Für `laune_faktor` kann ich
keinen Mittelwert mehr berechnen, da Vektoren vom Typ `factor`
...
Da die Berechnung nicht möglich ist, gibt `R` die folgende "Warnmeldung"
aus:
...
`NA`.[^`r incref()`] Da wir mit echten Datensätzen arbeiten, die oftmals
...
[^`r incfnc()`]: Eigentlich ist `NA` kein eigener Datentyp. In `R` hat
jeder Vektor **nur genau einen** Datentyp. Es ist beispielsweise nicht
...
*optionalen Argument* `na.rm`[^`r incref()`] der Funktion `mean`,
welches wir auf `TRUE` setzen können. Mit dem *Argument* `na.rm` ("NA
remove") teilt man `mean` mit, dass `NA` Werte bei der Berechnung des
Mittelwerts nicht berücksichtigt werden sollen (andere Funktionen wie
`sd` und `var` haben auch das Argument `na.rm`):

[^`r incfnc()`]: Ein Argument heißt optional, wenn wir dafür keinen Wert
...
Standardwert des Arguments `na.rm` in der Funktion `mean` ist `FALSE`.
...
Wir können in `R` Eigenschaften von Vektoren mithilfe von logischen
Vergleichen erfragen. So kann man beispielsweise prüfen, welche Werte
eines numerischen Vektors (a) gleich, (b) größer (c) kleiner, (d) größer
gleich, (e) kleiner gleich oder (f) ungleich einem bestimmten Wert
sind. Diese Operationen sind fundamental für den weiteren Verlauf des
Seminars. Es wird sich gegebenenfalls lohnen, bei den Themen
Datenauswahl in Datentabellen noch einmal diesen Abschnitt zu
konsultieren (in Kapitel [3](#subset)). Dieser Code-Abschnitt stellt die
grundlegenden logischen Vergleiche dar:
...
Das Ergebnis dieser Operationen ist ein Vektor aus `TRUE` und `FALSE`
Werten. Die Werte nehmen `TRUE` an, wenn die Zahlen die
kleiner/größer/gleich Bedingung erfüllen -- andernfalls
`FALSE`. **Beachtet, dass auf Gleichheit mit dem "doppelten" `==`
Operator getestet wird und nicht mit einem einfachen `=`.** Dies ist
eine häufige Quelle von Fehlern, die schwierig zu entdecken
sind. Betrachtet etwa folgenden Code -- was geht hier schief?
...
[^`r incfnc()`]: Für Vektoren vom Typ `factor` lässt sich auf dieselbe
Art und Weise eine Abfrage auf Gleichheit umsetzen.
...
Verteilungen zu "samplen". Mit `rnorm` lassen sich Zufallszahlen
generieren, die einer Normalverteilungen folgen; wenn man keine weiteren
...
`head` ist sehr praktisch, um sich schnell einen Blick über Daten zu
...
## Doppelter Code  wegen Formatierung =/ oben nur Ausgabe in Skript
## Male Histogram
hist(zufallsdaten, freq = FALSE,
     main = "Schöne normalverteilte Daten",
     xlab = "", ylab = "", las = 1,
...
ist als 1, erhalten wir `TRUE`, andernfalls `FALSE`. `sum` gibt die Zahl
der `TRUE` Einträge aus. **Das funktioniert, da `TRUE` und `FALSE` eine
numerische Interpretation haben: `TRUE` wird als `1` interpretiert und
`FALSE` als `0`**.[^`r incref()`]

[^`r incfnc()`]: Wenn logische Vektoren einer numerischen Berechnung
...
beispielsweise auch folgender Befehl:  
`TRUE * 2`  
`[1] 2`
...
`pnorm` herausfinden:[^`r incref()`]

[^`r incfnc()`]: `pnorm` ist die kummulative Verteilungsfunktion der
Normalverteilung. Sie sagt aus, wie viel % der Werte in einer
...
wenn ihr mehrfach `mean(rnorm(1000)>1)` und `mean(rnorm(100000)>1)` in
die `R`-Konsole eingebt und beobachtet, welcher Wert häufiger näher an
0.159 liegt. Beachtet wie schnell `R` Operationen mit 100,000 Zahlen
durchführen kann.

### Der `%in%` Operator

Um zu testen, ob ein oder mehrere Element in einem Vektor enthalten ist,
kann man den `%in%` Operator verwenden. Der sieht zwar
...
`%in%` testet für jedes der Elemente *vor* dem `%in%`-Operator, ob
dieses im Vektor *nach* dem `%in%`-Operator enthalten ist:
...
die Grundlagen `R`s. In diesem Abschnitt lernen wir, wie wir Elemente
aus einfachen Vektoren "herausgreifen" können.  Allgemein ist das
gezielte Auswählen von Daten ein wichtiger Bestandteil von `R`, häufig
etwa von Zeilen und Spalten aus Datentabellen, wie wir auch im nächsten
Kapitel lernen werden.

### Der `[·]`-Zugriff {#veczugriff}


Daten können mit dem `[·]`-Zugriff[^`r incref()`] *indexbasiert* aus
...
[^`r incfnc()`]: Ich nenne diese Operation `[·]`-Zugriff , da zur
...
`auswahl` ist ein logischer Vektor, der kodiert, welche Elemente des
Vektors `meinVektor` größer als 5 sind (spezifisch: welche Positionen
des Vektors `meinVektor` ein Element enthalten, das größer ist als
5). Ich kann nun den `[·]`-Zugriff mithilfe von `auswahl` verwenden, um
nur die Elemente auszuwählen, die größer sind als 5:
...
einem Fragebogen mit `-99` kodiert.[^`r incref()`] Wir wollen `R`
mitteilen, diesen Wert als fehlend zu interpretieren. Hier kommt uns
wiederum eine logische Abfrage zugute:

[^`r incfnc()`]: Das macht beispielsweise Sinn, damit bei der Eingabe
...
zum Umkodieren von fehlenden Werten werden wir in einer sehr ähnlichen
Form umsetzen, da wir sonst die Daten des Narcissistic Personality
Inventory nicht auswerten können. Bevor die Analyse starten kann, müssen
fehlende Werte gekennzeichnet werden.

## Zusammenfassung

- Wir haben `R`s grundlegendste Datenstruktur, den Vektor, kennengelernt
