...
Funktionen Gebrauch gemacht. Wir haben beispielsweise mit `subset` Daten
ausgewählt, mit `table` und `tapply` deskriptive Statistiken
...
In Zeile 5 wird mit der Funktion `return`[^imaketherules] angegeben,
...
[^imaketherules]: Es ist nicht unbedingt nötig, `return` zur Definition
von Rückgabewerten zu verwenden. Aber ich mache es so und verrate die
...
lang werden.[^codestil] Am Ende interessiert mich aber nur der
