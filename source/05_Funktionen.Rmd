...
und ihre Rückgabewerte in Variablen gespeichert werden können. In diesem
...
berechnet.[^`r incref()`] Bei Funktionen interessiert uns in erster
...
2. Wenn Argumente keinen Standardwert haben, **müssen** wir dem Argument
   ...
   kein Ergebnis ausgibt.[^`r incref()`]
...
[^`r incfnc()`]: Das ist streng genommen auch wieder eine Vereinfachung,
aber für uns reicht dieses Konzept: Hat ein Argument einen Standardwert,
ist es optional; hat ein Argument keinen Standardwert, ist es
verpflichtend.
...
ist dabei so gewählt, dass man ihn per Copy & Paste auch selbst in der
...
Platzhalterargument "..."  (lies: *ellipsis*) zu erkennen, siehe:[^`r
incref()`]
...
*t*-Test, sondern ein "Welch Two Sample t-test" gerechnet wurde.[^`r
incref()`] Weiterhin werden uns t-Wert, Freiheitsgrade, p-Wert
...
Elemente von beliebigem Typ und beliebiger Anzahl beinhalten können.[^`r
...
verschwinden nach Aufruf der Funktion wieder.[^`r incref()`] Es ist also
nicht so, dass die Variablen `numerator`, `denominator` und
`corrected_reliability` in die globale `R`-Umgebung geschrieben werden,
wenn ich die Funktion `spearman_brown` aufrufe. Das ist extrem nützlich:
Ich habe die Variablen `numerator` und `denominator` nur definiert,
damit mein Code gut lesbar ist und die einzelnen Code-Zeilen nicht zu
lang werden.[^`r incref()`] Am Ende interessiert mich aber nur der
...
Wenn ich eine Funktion schreibe, bleiben solche Hilfsvariablen
verborgen und nur der Rückgabewert dringt nach außen. Ich kann den
Rückgabewert abfangen, indem ich ihn in einer Variablen abspeichere:

[^`r incfnc()`]: Umgekehrt gilt hingegen: Innerhalb einer Funktion kann
...
[^`r incfnc()`]: Es ist guter Stil, die Menge von Code pro Zeile zu
...
Werte ableite -- machen eigene Funktionen oftmals mehr Sinn.
