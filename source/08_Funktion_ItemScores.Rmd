...
score_items6 <- function(
  ...
  if (ncol(df) != length(keys)) {
    stop("Die Zahl der Spalten in `df` passt nicht zur Zahl der Elemente in `keys`.")
  }
  # ...
}
```
...
wenn die relevanten Spalten in der Funktion selbst ausgewählt werden.
- Dokumentiert eure eigene Funktion `score_items7()`, indem ihr eure 
Dokumentation der Funktion `score_items5()` anpasst. 
- Fügt in die Funktion `score_items7()` Überprüfungen der Eingabe ein. Wenn ihr 
Fehler in der Eingabe feststellt, nutzt die Funktion `stop()` (`?stop`), um 
einen Fehler mit sinnvoller Fehlermeldung auszugeben. Zu den oben diskutierten 
Fehlerprüfungen macht noch die folgende Sinn: Passen die Namen des Vektors 
`keys` zu den Spaltennamen in `df`? Hier ist gegebenenfalls etwas Rechercher 
nötig, um herauszufinden, wie die Namen zwischen den beiden Variablen 
abzugleichen sind.
