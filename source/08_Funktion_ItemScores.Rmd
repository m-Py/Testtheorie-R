...
Bepunktung von Testitems umsetzt. Dabei kombinieren wir Inhalte aus den letzten 
beiden Kapitel zu Funktionen und zu Schleifen. Ich gehe vor allem auf die 
Entscheidungen ein, die beim Schreiben einer Funktion getroffen werden müssen; 
die rein technischen Aspekte des Schreibens einer Funktion [haben wir 
behandelt](#funktionschreiben).
...
von der konkreten Problematik einer einzelnen Aufgabe, zumeist heißt das, dem 
Datensatz, den wir gerade bearbeiten. Stattdessen versuchen wir die Essenz des 
...
Der nächste Schritt, den Code tatsächlich in eine Funktion umzuwandeln, macht 
ihn noch deutlich nützlicher. In einer Funktion wandeln wir die zwei Variablen 
aus dem obigen Code in Argumente um. Sie sieht dann wie folgt aus: 
...
auch ganz anders aussehen können. Damit meine ich nicht nur meine spezifische 
Implementierung der Funktionalität -- also den Code im Körper der Funktion --, 
sondern auch deren Bedienung, das heißt welche Argumente die Funktion annimmt 
und wie deren Rückgabewert aussieht. 
...
Für den Fall, dass die Funktion keine Spaltenauswahl durchführen soll, können 
wir das Argument wie folgt optional machen: 
...
werden.[^docsseiteneffekte] Entsprechend des Black-Box-Modell ist es weniger 
wichtig die interne Funktionsweise einer Funktion zu dokumentieren. Erklärende 
Kommentare im Code von Funktionen sind aber -- wie immer -- nützlich. 

[^docsseiteneffekte]: Sollte die Funktion Seiteneffekte haben, müssen sie 
natürlich dokumentiert werden. Alle hier diskutierten Funktionen haben keine 
Seiteneffekte. Das heißt: das einzige, was sie nach außen abgegeben, ist der 
explizit vorgesehene Rückgabewert.
...
#          Eine 1 zeigt an, dass die Antwort dem Schlüssel entsprach, 
...
technische Beschreibung: Welche Datenstruktur (`data.frame`, Vektor, ...) muss 
jeweils welchem Argument übergeben werden?
...
Funktionen reicht es -- wie hier im Beispiel -- eigene Funktionen informell mit 
Code-Kommentare zu dokumentieren. 

## Validierung der Eingabe {#inputvalidation}

Es ist gute Praxis, beim Schreiben von Funktionen ein paar Überprüfungen 
einzuführen, die sicherstellen, dass alle Argumente korrekt übergeben wurden. 
Solche Überprüfungen verhindern potenzielle Fehler. Mögliche Überprüfungen 
könnten im vorliegenden Fall sein: 

- Hat der Vektor `keys` so viele Einträge wie das Argument `df` Spalten hat? Bei 
den Varianten 5 und 6 müsste dies **nach** der Spaltenauswahl geprüft werden, da 
ja potentiell Spalten ausgeschlossen werden.
...
In dieser "finalen" Version habe ich noch eine zusätzliche Überprüfung der 
Eingabe eingebaut (passt die Angabe der Spaltennamen zum Argument `df`). 
Außerdem enthält die Funktion nun sowohl das Argument `column_names` als auch 
das Argument `selection`. Betrachtet genau, wie diese Argumente nun in der 
Funktion verarbeitet werden. 

Der Unterschied zu den vorherigen Versionen der Funktion, auf den ich vor allem 
aufmerksam machen möchte, ist aber noch ein anderer. Betrachtet die Art, in der 
nun die Spalten adressiert werden: Hier verwende ich die `[·,·]`-Notation und 
nicht mehr die `[[·]]`-Notation zur Auswahl von Spalten aus `df`. Der Grund 
hierfür ist, dass die `[·,·]`-Notation allgemeiner ist. Sie funktioniert auch 
dann, wenn die Eingabe `df` eine Matrix ist und kein `data.frame`. Eine Matrix 
ist eine weitere Datenstruktur in `R`, die strukturell äquivalent zu einem 
`data.frame` ist: Sowohl Matritzen als auch `data.frames` sind zweidimensionale 
Tabellen bestehend aus Zeilen und Spalten. Jedoch muss in einer Matrix jede 
Spalte -- und damit sogar jede einzelne Zelle -- denselben Datentyp haben, wie 
wir es aus einem Vektor kennen (siehe auch die Hilfe in `?matrix`). Wenn 
Antworten auf Testitems numerisch kodiert sind, können diese also sowohl als 
`data.frame` als auch als Matrix vorliegen.
...
Beginn die erste Spalte ab, danach die zweite Spalte, usw. Man sagt so, 
Matritzen werden in "column major order" abgespeichert. 
...
zurückgeben, ist es nützlich diesen Datentyp zu kennen (wobei dies nicht der 
einzige Grund ist, mit Matritzen zu arbeiten).
...
Stellt erst einmal sicher, dass `keys` überhaupt ein benannter Vektor ist. 
Weiterhin prüft, ob die Namen des Vektors `keys` zu den Spaltennamen in `df` 
passen. Hier ist gegebenenfalls etwas zusätzliche Recherche nötig, um 
herauszufinden, wie die Namen zwischen den beiden Objekten abgeglichen werden 
können. 
