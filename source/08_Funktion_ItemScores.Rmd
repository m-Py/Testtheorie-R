...
Kenntnisse vor: Wir schreiben eine Funktion, die für beliebige Datensätze die 
...
Testinventaren auftreten kann und nicht nur für eine einzelne Datenauswertung 
relevant ist.
...
demographischen Angaben. Einer Funktion würden wir einen reduzierten data.frame 
übergeben, in dem nur die Spalten der Items ausgelesen wurden -- alle anderen 
Spalten sind für das Problem nicht relevant. 

Im Folgenden Beispiel habe ich den problemspezifischen Code schon einmal etwas 
...
```{r}
score_items5 <- function(
  df, keys, column_names = paste0(colnames(df), "_recoded"), selection
) {
  df <- df[, selection]
  for (i in 1:ncol(df)) {
    df[[i]] <- ifelse(df[[i]] == keys[i], 1, 0)
...
Eine Konvention in der `R`-Community ist es jedoch diese Art von optionalen 
...
Bei den Varianten 5 und 6 müsste dies **nach** der Spaltenauswahl geprüft werden.
...
Bei solchen Überprüfungen kommen die `if{}/else{}`-Schlagwörter zum Einsatz. 
Wenn ein Fehler festgestellt wird, kann die Funktion `stop()` genutzt werden, um 
einen Fehler auszugeben. Es bietet sich etwa folgende Überprüfung an: 
...
In diesem Fall wird der Aufruf der Funktion abgebrochen, wenn die Argumente `df` 
und `keys` nicht zusammenpassen. Dies stellt die Funktion `stop()` sicher, die 
in diesem Fall auch eine diagnostische Fehlermeldung ausgibt. 

Ich werde an dieser Stelle nicht tiefer auf die Überprüfung von Argumenten 
eingehen. Denkt aber daran, dass solche Überprüfungen wichtig sind und euch 
gravierende Fehler ersparen können. Welche Prüfungen Sinn machen, hängt immer 
von dem konkreten Anwendungsfall ab.

## Abschließender Hinweis: Matritzen

Wer die oben definierte Funktion zur Bepunktung von Items selbst nutzen will, 
verwendet besser folgende Abwandlung: 
...
score_items <- function(
  ...
  # Optional: Auswahl der relevanten Spalten:
  if (!is.null(selection)) { # `selection` wurde spezifiziert
    df <- df[, selection]
  }
...
Der einzige Unterschied: Hier verwende ich die `[·,·]`-Notation statt der 
`[[·]]`-Notation zur Auswahl der Spalten. Der Grund hierfür ist, dass erstere 
allgemeiner nutzbar ist. Sie funktioniert auch dann, wenn die Eingabe eine 
Matrix ist (siehe `?matrix`) und kein `data.frame`. Eine Matrix ist strukturell 
äquivalent zu einem `data.frame`, also eine zweidimensionale Tabelle bestehend 
aus Zeilen und Spalten. Jedoch muss in einer Matrix jede Spalte -- und damit 
sogar jede einzelne Zelle -- denselben Datentyp haben, wie wir es aus einem 
Vektor kennen. Wenn Antworten auf Testitems numerisch kodiert sind, können diese 
also sowohl als `data.frame` als auch als Matrix vorliegen.
...
[^matrixistvektor]: Technisch gesehen ist eine Matrix sogar ein Vektor, dem 
intern nur die Dimensionen Reihe/Spalte zugeordnet werden. Die `[[·]]`- und die 
`[·]`-Notationen betrachten die Matrix als normalen eindimensionalen Vektor. Die 
Befehle `mat[2]` oder `mat[[2]]` geben das zweite Element -- und nicht die 
zweite Spalte -- in diesem zugrundeliegenden Vektor aus. Eine interessante ist 
nun: Was ist in einer Matrix das *zweite* Element? Möglich wäre: Der Wert in der 
ersten Zeile, zweite Spalte, oder der Wert in der zweiten Zeile, erste Spalte. 
In `R` ist letzteres der Fall, also der Wert in der zweiten Zeile. Man sagt, 
Matritzen werden in "column major order" in Vektorenform abgespeichert. Der 
zugrundeliegende Vektor speichert zu Beginn erste Spalte ab, danach die zweite 
Spalte, usw.

Matritzen sind stellen in `R` eine wichtige Datenstruktur dar. Beispielsweise 
gibt die Funktion `cor()`, wenn man die Inter-Itemkorrelationen zwischen 
mehreren Items anfordert, eine Matrix und keinen `data.frame` zurück. In solchen 
Fällen müssen wir auf der Hut sein; ein typischer Fehler ist der Versuch, mit 
der `$`-Notation Spalten aus Matritzen auszulesen, was fehlschlägt. Wir haben 
bislang mit `data.frames` gearbeitet, da Forschungsdaten im Normalfall immer als 
`data.frame` vorliegen -- man benötigt eben im Normalfall mehrere Datentypen in 
einer Datentabelle. Da einige Funktionen aber Matritzen zurückgeben, ist es 
nützlich diesen Datentyp zu kennen. 

## Fragen und Übungen zum vertiefenden Verständnis

1. Dokumentiert die Funktion `score_items5()`, indem ihr das 
Dokumentationsschema der Funktion `score_items4()` anpasst.
2. Ändert die Funktion `score_items6()` in eine neue Variante `score_items7()` 
um. Nun soll `keys` [ein benannter Vektor](#nameindex) sein, der dieselben Namen 
hat, wie die relevanten Spalten der Items im Argument `df`. Im Körper der 
Funktion findet dann die Auswahl sowohl der Spalten in `df` als auch der 
Einträge in `keys` [nach Name](#nachname) statt. Solche Adressierung nach Namen 
ist im allgemeinen weniger fehleranfällig als die Adressierung nach Index -- 
insbesondere dann, wenn die relevanten Spalten erst in der Funktion selbst (mit 
dem Argument `selection`) ausgewählt werden.
3. Dokumentiert eure eigene Funktion `score_items7()`, indem ihr eure 
Dokumentation der Funktion `score_items5()` anpasst. 
4. Fügt in die Funktion `score_items7()` einige Überprüfungen der Eingabe ein. 
Wenn ihr Fehler in der Eingabe feststellt, nutzt die Funktion `stop()` 
(`?stop`), um einen Fehler und eine sinnvolle Fehlermeldung auszugeben. Zu den 
oben diskutierten Fehlerprüfungen macht noch die folgende Sinn: Passen die Namen 
des Vektors `keys` zu den Spaltennamen in `df`? Hier ist gegebenenfalls etwas 
Recherche nötig, um herauszufinden, wie die Namen zwischen den beiden Objekten 
