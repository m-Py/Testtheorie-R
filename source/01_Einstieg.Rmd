...
Die ersten Schritte mit `R` als Programmiersprache nehmen wir, indem wir
**`R`-Code** in die sogenannte Konsole eingeben. Im Normalfall finden
wir die `R`Konsole in RStudio im Panel auf der linken Seite (je nachdem,
wie ihr RStudio geöffnet habt, befindet sich die Konsole auch unten
links). Wir erkennen die Konsole daran, dass die Zeile, in die wir Text
eintragen können, mit einem `>` beginnt. Um euren  ersten `R`-Befehl
auszuführen, schreibt Folgendes in die Konsole und drückt `Enter`:
