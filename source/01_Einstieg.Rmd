...
Diagnostik bzw. der Testtheorie kennen. Dabei werden auch echte
...
Programmiersprache `R` nicht immer Rücksicht nehmen. Das Skript so
ausgelegt, dass ein Grundstein an Kenntnissen gelegt wird, jedoch die
Meisterung von `R` noch weitere eigenständige Einarbeitung erfordert.
...
*rmarkdown* [1.8, @R-rmarkdown], und *tufte* [0.2, @R-tufte] genutzt.
...
"install R for the first time" gehen. Wenn ihr RStudio und `R`
heruntergeladen habt, startet RStudio und schreibt Folgendes in die
`R`-Konsole und drückt `Enter`:
...
Befehle können wir in `R` in die Konsole eingeben. Wir können das als
...
korrekter `R`-Befehl war**. Andernfalls gibt uns `R` eine Fehlermeldung
zurück. Zum Beispiel können wir die `R`-Konsole als Taschenrechner
benutzen:
...
Datentabellen darstellen, wie wir sie auch aus Excel oder SPSS
kennen. In [Kapitel 4](#psychometrie) werden wir psychometrische
Datenauswertungen durchführen und dabei das Wissen anwenden, das wir
zuvor erworben haben.
