...
Ich danke Juli Nagel für ihre wertvollen Beiträge und ihr nützliches Feedback
...
Gerade die Arbeit mit Zusatzpaketen erfordert, dass die installierte `R`-Version 
nicht zu alt sein sollte. Auf manchen älteren Rechnern (gerade auf Macs) scheint 
es jedoch manchmal ein Problem zu sein, eine aktuelle `R`-Version zu 
installieren. In dem Fall kann es eine Alternative sein, eine Cloud-basierte 
Installation zu nutzen, etwa [Posit Cloud](https://posit.cloud/), das RStudio im 
Browser nutzbar macht.
...
Es ist ebenfalls nützlich, zu überprüfen, ob die Installation von Zusatzpaketen 
reibungslos funktioniert. Für die psychometrische Auswertung ist beispielsweise 
das Paket `psych` verbreitet, das wir mit dem Befehl `install.packages()` 
installieren können: 
