...
auch Spalten vom Typ `factor` oder `character` im `data.frame` gehabt,
hätte ich Funktionen wie `rowSums` und `colMeans` nicht einfach auf den
ganzen `data.frame` anwenden können.[^subsetistsuper]

[^subsetistsuper]: In dem Fall könnte man mit der `[·,·]`-Notation oder
der Funktion `subset` nur die gewünschten Spalten auswählen.
...
"Kriterium" -- also den Gesamt-Testscore -- eingeht.[^diskrimination]
Eine Möglichkeit, die "part-whole" korrigierte Trennschärfe für eine
Item (hier: Item 2) zu berechnen, bietet der folgende Code:
...
`[·,·]`-Notation und den Funktionen `paste0` und `rowSums` lässt sich
allgemein mit beliebig vielen Items durchführen. Da wir an dieser Stelle
nur eine Summe über vier Items bilden, hätte auch der folgende --
simplere -- Code funktioniert:
