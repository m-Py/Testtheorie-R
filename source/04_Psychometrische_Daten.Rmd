...
numerische Funktion auf den ganzen `data.frame` angewendet habe. Hätte
der `data.frame` auch Spalten vom Typ `factor` oder `character`
enthalten, ist es nicht möglich Funktionen wie `rowSums` und `colMeans`
auf den `data.frame` anzuwenden. In dem Fall könnte man mit der
...
nur 5 Items kann der Effekt substantiell sein.
