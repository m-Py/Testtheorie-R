...
In diesem Abschnitt lernen wir die Funktion `subset` kennen, die --
verglichen mit der `[·,·]`-Notation -- zwei wichtige Möglichkeiten des
Datenzugriffs auf `data.frames` vereinfacht:

1. [Die Auswahl von Fällen anhand logischer Bedingungen](#zeileneinfach)
2. [Die Auswahl von Spalten durch Angabe der Spaltennamen](#spalteneinfach)

Desweiteren bietet dieser Abschnitt einen ersten Überblick über die
[Arbeitsweise von Funktionen](#teaserfunktionen), der in [Kapitel
5](#funktionen) fortgeführt wird.

### Vereinfachte Zeilenauswahl {#zeileneinfach}

Die Funktion `subset` erlaubt uns logische Bedingungen zur Zeilenauswahl
zu formulieren, ohne die `$`-Notation zu verwenden:
...
Variable, sondern nur eine Spalte von `mdf`.[^`r incref()`] Innerhalb
der Funktion `subset` kann die logische Bedingung in dieser Form jedoch
verarbeitet werden; durch Angabe von `mdf` im ersten Argument ist ja
klar, aus welchem `data.frame` Zeilen ausgewählt werden sollen. 
...
Die Verknüpfung logischer Bedingungen funktioniert entsprechend:
...
auszulesenede Spalten adressiert:

```{r}
subset(mdf, Augenfarbe == "blau", c("Item1", "Augenfarbe"))
```

Durch die Kombination der Auswahl von Zeilen und Spalten wird ein
`data.frame` ausgegeben, der nur die Spalten `Item1` und `Augenfarbe`
enthält, und diese nur für Personen mit blauen Augen.

An dieser Stellen machen wir uns eine wichtige Eigenschaft von `R`
bewusst: **Funktionen erkennen Argumente anhand der Reihenfolge, in der
sie übergeben werden.** Bei der Funktion `subset` ist das erste Argument
der `data.frame`, von dem wir Daten anfordern. Das zweite Argument wählt
mit einem logischen Ausdruck Zeilen aus. Das dritte Argument wählt
Spalten aus. Was passiert, wenn wir die Argumente in einer anderen
Reihenfolge angeben?

```R
subset(mdf, c("Item1", "Augenfarbe"), Augenfarbe == "blau")
```
...
ist der Fehler klar: Das zweite Argument von `subset` muss eine logische
Bedingung zur Auswahl der Zeilen enthalten, wir haben stattdessen aber
einen Vektor zur Spaltenauswahl übergeben. `R` verlangt, dass
Funktionsargumente genau in der richtigen Reihenfolge übergeben werden.

Was könnten wir aber tun, wenn wir **nur** eine Auswahl nach Spalte
ausführen wollen? Wenn wir also gar keine Auswahl nach Zeilen
durchführen wollen und demnach gar kein zweites Argument angeben wollen.
Wir können das zweite Argument ja nicht leer lassen, denn das führt zum
obigen Fehler. Dieses Problem lässt sich mit einer praktischen
Eigenschaft von `R` lösen: **Die Argumente von Funktionen haben Namen.**
Bislang haben wir das ignoriert bzw. es ist uns nur am Rande begegnet
(erinnern wir uns an das Argument `na.rm` der Funktion `mean`).
...
Die `R`-Hilfe informiert uns über die Argumente, die Funktionen annehmen
können. Leider ist diese Hilfe oftmals kryptisch -- und das nicht nur
für Anfänger. Sie ist die offizielle Dokumentation von Funktionen und
legt deswegen zwar großen Wert auf technische Genauigkeit, ist aber
nicht immer sonderlich ausführlich oder gar verständlich. Wir werden in
[Kapitel 5](#funktionen) bei einer ausführlicheren Besprechung von
Funktionen noch einmal darauf zurückkommen, wie wir mit der
Hilfe-Funktion umgehen können.
...
also solche, die wir nicht immer angeben müssen. Wir können
beispielsweise das Argument `select` weglassen, wenn wir eine Auswahl
nach Zeilen, aber nicht nach Spalten treffen wollen. Umgekehrt muss ich 
das Argument `subset` nicht angeben -- in dem Fall werden alle Zeilen 
...
Mit diesem Aufruf werden mir alle fünf Fälle von `mdf` zurückgegeben, aber 
für diese nur der Testscore und das Geschlecht. Wie dieser Aufruf zeigt, 
kann ich Auswahl nach Position und Auswahl nach Namen mischen. Für das 
erste Argument -- `mdf` -- habe ich den Argumentnamen `x` nicht
...
Die Funktion `subset` bietet uns einige Sonderregeln zur Auswahl von
Spalten, die über die Auswahl per `character`-Vektor hinausgehen. 

Zunächst ermöglicht uns die Funktion `subset` Spaltennamen ohne
...
Wie auch die vereinfachte Zeilenauswahl, die ohne die `$`-Notation
auskommt, ist dies eine Besonderheit der Funktion `subset`: Der
Ausdruck `c(Augenfarbe, Alter)` würde außerhalb der Funktion einen
Fehler verursachen, da `Augenfarbe` und `Alter` nicht unbedingt als
Variablen definiert sind; sie sind bloß Spalten von `mdf`. 

Dieser Umstand ist noch keine allzu große Verbesserung zur
Spaltenauswahl mit der `[·,·]`-Notation; es erlaubt uns bloß
Anführungszeichen bei der Angabe der Spalten auszulassen -- immerhin.
Die Funktion `subset` lässt aber noch weitere Sonderfälle zu, die das
Leben erleichtern können. So können wir den Doppelpunktoperator
verwenden, um mehrere Spalten anzusteuern, etwa wie folgt:

```{r}

## Wähle alle Spalten zwischen den Spalten `Item1` und 
## `Geschlecht` aus:
subset(mdf, select = Item1:Geschlecht)
```

Diese Funktionalität ist beispielsweise bequem, um alle Items eines
Testinventars auszuwählen.
...
danach erst einmal nicht weiter verwenden, kann dieser Abschnitt jedoch
...
wir sie hingegen nicht. Das hat zur Folge, dass wir statt
...
angeben. Das ist mit den Zugriffen per `[[·]]` und `$` nicht möglich,
...
Wenn wir nur eine Spalte auslesen, gibt die `[·,·]`-Auswahl einen Vektor aus,
die `[·]`-Auswahl jedoch einen `data.frame` mit einer Spalte. Dieses Verhalten
offenbart einen Sonderfall der `[·,·]`-Auswahl: Im Normalfall gibt `[·,·]`
ebenfalls einen ganzen `data.frame` aus. Wenn wir aber nur eine einzige Spalte
anfordern, "reduziert" sich die Ausgabe zu einem Vektor.


### Zugriff nach Name und Index {#nameindex}

Wir haben nun alle wichtigen Möglichkeiten kennengelernt, Zugriffe auf
`data.frames` durchzuführen. An dieser Stelle lohnt es sich deswegen, ein
grundsätzliches Prinzip von Datenzugriffen in `R` festzustellen:
**Datenzugriffe können nach nach Index oder nach Name stattfinden**.
Dies gilt für Vektoren, `data.frames` und auch andere Datenstrukturen,
...
explizit mit einem numerischen Vektor angegeben haben, oder (b) indem
wir einen logischen Vektor übergeben haben, der anhand von `TRUE` und
`FALSE` Werten die Indexe auswählt, deren Elemente ausgegeben werden
sollen. Der Vollständigkeit halber sei hier mitgeteilt, dass man sogar
bei Vektoren Zugriffe nach Namen durchführen kann, wenn die Elemente des
Vektors benannt sind. Das ist gar nicht so ungewöhnlich; wie folgt
könnte man einen Vektor mit benannten Elementen erstellen und mit der
bekannten `[·]`-Notation darauf zugreifen.
