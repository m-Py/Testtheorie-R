...
In diesem Abschnitt nutzen wir die Funktion `subset`, um die zwei
wichtigsten Möglichkeiten des Datenzugriffs auf `data.frames` zu
vereinfachen:

1. Die Auswahl von Fällen anhand logischer Bedingungen
2. Die Auswahl von Spalten durch Angabe der Spaltennamen

Mit der Funktion `subset` können wir mit logischen Bedingungen Fälle aus
einem `data.frame` auswählen, ohne die`$`-Notation zu verwenden:

```{r}
subset(mdf, Augenfarbe == "blau")
```

Dieses Verhalten ist eine Besonderheit der Funktion `subset`; außerhalb
der Funktion würde der Befehl `Augenfarbe == "blau"` einen Fehler
ausgeben, da `Augenfarbe` selbst keine Variable ist -- nur eine Spalte
von `mdf`.[^`r incref()`] Innerhalb der Funktion `subset` kann die
logische Bedingung in dieser Form verarbeitet werden, da als erstes Argument der
`data.frame` angegeben wurde, aus dem die Daten ausgewählt werden
sollen. Das Folgende ist also nicht nötig, aber es funktioniert auch:
...
Mit dem dritten Argument können wir Spalten auswählen:

```{r}
subset(mdf, Augenfarbe == "blau", c("Item1", "Augenfarbe"))
```

Hier habe ich mit `c('Item1', 'Augenfarbe')` die gewünschten Spalten
übergeben; dieser Vektor vom Typ `character` adressiert sie mit Namen. 
Durch die Kombination der Auswahl von Zeilen und  Spalten wird mir 
insgesamt ein `data.frame` ausgegeben, der nur die Spalten "Item1" und 
...
zurück -- selbst dann, wenn ich nur eine einzige Spalte anfordere.
```

Bei dieser Verwendung der Funktion `subset` fällt eine allgemeine
Eigenschaft von Funktionen auf: Sie erkennen anhand der Reihenfolge 
der Argumente, wie sie sich zu verhalten haben. Im Fall von `subset`
übergibt das erste Argument den `data.frame`, von dem wir Daten 
anfordern. Das zweite Argument wählt mit einem logischen Ausdruck 
**Zeilen** aus, das dritte Argument wählt durch einem `character`-Vektor 
**Spalten** aus. Was passiert, wenn wir diese Reihenfolge ändern?
...
der Zeilen beschreiben, wir haben stattdessen aber einen
`character`-Vektor übergeben, der die Spalten auswählen sollte. Was
können wir tun, wenn wir **nur** eine Auswahl nach Spalte ausführen
...
verständlich. Wir werden in [Kapitel 5](#help) bei einer ausführlicheren
...
adressieren. "`Wert`" ist dabei immer ein `R`-Objekt und
