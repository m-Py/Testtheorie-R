...
nutzen ihn, um auf einzelne Spalten eines `data.frames` zugreifen und
diese **als Vektor** auslesen:
...
falsche Daten abspeichern und nicht einmal einmal eine Fehlermeldung
erhalten.
...
Messvariablen berechnen oder uns Häufigkeiten von katogegorialen Daten
angeben lassen:
...
solche Auswahlen hilft uns unser Wissen über [logische Vergleiche aus
dem letzten Kapitel](#logischevergleiche). Betrachten wir zunächst ein
Beispiel:[^nurausgabe]
...
bewirkt, ob eine Auswahl nach Zeilen oder Spalten stattfindet. **Vor dem
Komma werden Zeilen adressiert, nach dem Komma Spalten.** Es ist auch
eine gleichzeitige Auswahl nach Zeilen **und** Spalten möglich.
...
Desweiteren bietet der Abschnitt einen ersten allgemeinen Überblick über
...
Argument ist ja klar, aus welchem `data.frame` Zeilen ausgewählt werden
sollen.
...
**Das war auch nötig**, da das zweite Argument ansonsten die Auswahl der
Zeilen erwartet hätte.

In `R` kann man **fast immer**[^vieleargumente] Argumente per Position
und per Name ansprechen. Oftmals wollen wir die Namen von Funktionen
...
ausgegeben, aber nur eine Teilmenge der Spalten. Ähnlich ist es bei der
Funktion `mean`: Das Argument `na.rm` geben wir normalerweise nur an,
wenn ein Vektor fehlende Werte enthält.
...
hinausgehen, die wir von der `[·,·]`-Notation kennen. Zunächst einmal
können wir Spaltennamen ohne Anführungszeichen anzugeben:
...
selbst wenn ich gar nicht weiß, was in dem Objekt drin steckt, auf dem
die Auswahl stattfindet. Die `[·]`-Notation ist uneindeutiger: Sie
könnte auch auf einem Vektor operieren. Code ist in erster Linie für
Menschen gemacht, die ihn verstehen sollten. Deswegen ist verständlicher
Code gegenüber kürzerem Code zu bevorzugen.
