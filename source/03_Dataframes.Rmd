...
achten. Die Variablen,[^`r incref()`] die wir verwenden, um den
...
`R`-Variable, sondern nur eine Spalte von `mdf`.[^`r incref()`]
...
[^`r incfnc()`]: Eine Ausnahme bildet die Funktion `c`, für die wir
...
würde der Ausdruck `c(Augenfarbe, Alter)` höchstwahrscheinlich[^`r
incref()`] einen Fehler verursachen, da `Augenfarbe` und `Alter` nicht
unbedingt als Variablen definiert sind; sie sind bloß Spalten von `mdf`.

[^`r incfnc()`]: Unter welchen Umständen würde `R` keinen Fehler
