...
stattdessen Daten aus einer externen Datei einlesen.[^excel] Die
...
[^excel]: Beispielsweise können die Daten in einem
...
Bei der Erstellung des `data.frames` wurde jede Spalte mit der Funktion
`c` oder dem Doppelpunktoperator mit genau einem Vektor befüllt. Die
Spalten des `data.frames` wurden bei der Erstellung benannt. **Dieser
Punkt ist sehr wichtig, da wir Spalten anhand ihrer Namen gezielt
auswählen können**. Wenn ich die Spaltennamen eines `data.frames` nicht
mehr weiß, kann ich sie mit der Funktion `names` abrufen:
...
nutzen ihn, um auf einzelne Spalten eines `data.frames` zuzugreifen und
diese **als Vektor** auszulesen:
...
die Häufigkeiten aller Werte, die in einem Vektor vorkommen. Wir nutzen
`table` vor allem zur Beschreibung kategorialer Messvariablen. Auch zur
Überprüfung der Plausibilität von Daten ist `table` nützlich. (Ist jeder
Wert ein "legaler" Wert, der auch vorkommen sollte?) Ich kann die
Funktion `table` auch verwenden, um die Häufigkeit der Kombination von
mehreren Variablen zu erfragen, etwa wie häufig welcher Testscore nach
Geschlecht auftaucht:
...
[^nurausgabe]: Beachtet, dass durch diesen Aufruf der `data.frame`,
...
dafür entscheident ist, ob eine Auswahl nach Zeilen oder Spalten
stattfindet. **Vor dem Komma werden Zeilen adressiert, nach dem
Komma Spalten.** Es ist auch eine gleichzeitige Auswahl nach Zeilen
**und** Spalten möglich. Allgemein ist die Syntax zum Ansprechen von
...
Zeile und dann -- nach dem Komma -- die Spalte adressiert. Man kann die
...
In diesem Abschnitt lernen wir die Funktion `subset` kennen, die zwei
wichtige Zugriffe auf `data.frames` vereinfacht:
...
Des Weiteren bietet der Abschnitt einen ersten allgemeinen Überblick über
die [Arbeitsweise von Funktionen](#teaserfunktionen), der in [Kapitel
5](#funktionen) vertieft wird.
...
`R`-Variable, sondern nur eine Spalte von `mdf`.[^innehalten]
Innerhalb der Funktion `subset` kann die logische Bedingung in dieser
Form jedoch verarbeitet werden. Durch Angabe von `mdf` im ersten
Argument teilen wir `subset` mit, aus welchem `data.frame` Zeilen
...
In `R` kann man **fast immer**[^vieleargumente] Argumente per Position
und per Name ansprechen. Oftmals wollen wir die Namen von
Funktionsargumenten explizit verwenden. Wie wir im Beispiel oben gesehen
haben, kann das Weglassen von Argumenten aber zu Fehlern führen, wenn es
die Identifikation der Argumente ihrer Reihenfolge nach verhindert.
Außerdem haben viele Funktionen *optionale* Argumente haben -- also
solche, die wir nicht immer angeben müssen. Dann können wir per Namen
nur die Argumente ansprechen, die wir auch nutzen wirklich wollen. Wir
können beispielsweise das Argument `select` weglassen, wenn wir eine
Auswahl nach Zeilen, aber nicht nach Spalten treffen wollen. Umgekehrt
muss ich das Argument `subset` nicht angeben -- in dem Fall werden alle
Zeilen ausgegeben, aber nur eine Teilmenge der Spalten. Ähnlich ist es
bei der Funktion `mean`: Das Argument `na.rm` geben wir normalerweise
nur an, wenn ein Vektor fehlende Werte enthält.

[^vieleargumente]: Eine Ausnahme bildet die Funktion `c`, für die wir
keine Funktionsnamen angeben. Hier gilt nämlich: Wir können beliebig
viele Vektoren als Argumente angeben; deswegen gibt es natürlich keinen
separaten Namen für jedes mögliche Argument. Feste Namen gibt es aber
dann, wenn es eine feste Anzahl an möglichen Argumenten gibt -- wie bei
der Funktion `subset`.
...
zuzsätzliches Komma verwenden muss (ansonsten sind die beiden Notationen
...
auch auf einem Vektor operieren. Code ist in erster Linie für Menschen
gemacht, die ihn verstehen sollten. Deswegen ist verständlicher Code
gegenüber kürzerem Code zu bevorzugen.
...
ein grundsätzliches Prinzip von Datenzugriffen in `R` festzustellen:
...
nicht nur fünf -- Datenpunkte hat. Das gilt gerade dann, wenn wir mehrere
...
sollen. Wenn wir *n* nicht angeben, werden sechs Zeilen ausgegeben (in
...
`data.frame` insgesamt nur fünf Zeilen hat -- und somit weniger als sechs.
...
- Wir haben den Zugriff auf einzelne Spalten `data.frames` mit der
  `$`-Notation und der `[[·]]` kennengelernt
- Mit der
