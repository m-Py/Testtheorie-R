...
Wie hier gezeigt wäre im Vergleich zur `[·,·]`-Notation jedoch noch 
nicht viel gewonnen. Was die Funktion `subset` so nützlich macht, ist 
dass sie zwei wichtige Zugriffe auf `data.frames` vereinfacht:
...
Folgendermaßen könnte man durch Verwendung von `$` einen äquivalent 
Aufruf durchführen:
...
verarbeitet werden. Durch Angabe von `mdf` im ersten Argument teilen wir 
`subset` mit, aus welchem `data.frame` Zeilen ausgewählt werden sollen.
