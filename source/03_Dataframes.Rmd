...
fehlen, also `NA` sind. Es ist äußerst nützlich sich bei der Arbeit mit einem 
neuen Datensatz erst einmal auf diese Weite einen groben Überblick zu 
verschaffen.

Zu nennen sind etwa folgende Funktionen: 

- Die Funktion `skim()` aus dem Paket `skimr`
- Die Funktion `describe()` aus dem Paket `psych`
- Die Funktion `describe()` aus dem Paket `Hmisc`
- Die Funktion `summary()`, schon ohne Zusatzpaket in `R` enthalten

Es ist nützlich all die genannten Funktionen auszuprobieren und zu entscheiden, 
welche einem davon am besten gefällt. Diese dann kann dann regelmäßig genutzt 
werden, um sich in neuen Datensätzen einen Überblick zu verschaffen. 

Um alle Funktionen auszuprobieren, können die nötigen Pakete können mit der 
Funktion `install.packages()` installiert und der Funktion  `library()` geladen 
werden:

```{r, eval = FALSE}
# Man kann mit Befehl mehrere Pakete installieren:
...
Beachtet an dieser Stelle noch einmal, dass man Zusatzfunktionen aus Paketen 
erst dann nutzen kann, wenn diese per `library()` geladen wurden; eine (fast) 
äquivalente Alternative zu `library()` stellt die Funktion `require()` dar.

Interessanterweise sehen wir an dieser Stelle ein mögliches Problem, das sich 
...
Funktion wird dann genutzt, wenn ich `describe()` aufrufe? Das ist a priori 
leider nicht zu sagen und hängt von der Reihenfolge ab, in der die Pakete 
geladen wurden. Wenn ich gezielt genau eine der beiden Funktionen ansteuern 
will, kann ich den doppelten Doppelpunktoperator nutzen, um das zugehörige Paket 
anzugeben:
