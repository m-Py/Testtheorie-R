...
aufweist: Auch hier kann ich die Spaltennamen ohne Anführungszeichen übergeben; 
ebenso ist keine Adressierung per `$`-Notation nötig. Dass das funktioniert ist 
gewiss nicht selbstverständlich. Normalerweise würden wir ja erwarten, dass 
ein Name ohne Anführungszeichen eine Variable sein muss, nicht bloß eine Spalte 
in einem `data.frame`. Dass es bei `arrange()` und `subset()` trotzdem 
funktioniert, ist einer mächtigen Eigenart von `R` zu verdanken, die unter dem 
Namen „Non-Standard Evaluation“ bekannt ist. Non-Standard Evaluation ist ein 
tiefes und sogar kontroverses Thema, das an dieser Stelle nur angesprochen, 
jedoch nicht ausgeführt wird. Für Interessierte sei das [entsprechende 
Kapitel](http://adv-r.had.co.nz/Computing-on-the-language.html) in Hadley 
Wickhams Buch „Advanced R“ [-@wickham2014] als Standardreferenz genannt; eine 
schnelle Internetrecherche wird ebenfalls einen Überblick zu dem Thema 
liefern.[^blogpost]

[^blogpost]: Etwa dieser Blog-Post diskutiert Non-Standard 
Evaluation anhand der Funktion `subset()`: 
https://www.brodieg.com/2020/05/05/on-nse/

Non-Standard Evaluation bietet die Basis einer prominenten Weiterentwicklung 
von `R`: dem [`Tidyverse`](https://www.tidyverse.org/) [Wickham et al., 
-@wickham2019]. Das Tidverse ist eine umfangreiche Sammlung von R-Zusatzpaketen 
-- von denen `dplyr` selbst wohl das bekannteste Mitglied darstellt --, die 
einem zugrundeliegenden Entwurfsprinzip folgen und viele Aufgaben der 
statistischen Datenanalyse vereinfachen sollen. 
