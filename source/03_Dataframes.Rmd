...
Desweiteren bietet der Abschnitt -- anhand des Beispiels der Funktion
`subset` -- einen ersten allgemeinen Überblick über die [Arbeitsweise
von Funktionen](#teaserfunktionen), der in [Kapitel 5](#funktionen)
vertieft wird.
...
angegeben -- das hat aber nicht funktioniert. Wir erhalten eine
...
uns eine wichtige Eigenschaft von `R` bewusst: **Funktionen erkennen
Argumente anhand der Reihenfolge, in der sie übergeben werden.** Bei der
Funktion `subset` ist das erste Argument der `data.frame`, von dem wir
Daten anfordern. Das zweite Argument wählt mit einem logischen Ausdruck
Zeilen aus. Das dritte Argument adressiert Spalten.
...
ist uns nur am Rande begegnet (erinnern wir uns an das Argument `na.rm`
der Funktion `mean`).
...
Wenn wir die Namen der Argumente der Funktion `subset` kennen,
können wir sie auch mit der folgenden Notation aufrufen:
...
-- daher wurde das Argument anhand seiner Position identifiziert. Für die
Auswahl der Spalten habe ich jedoch den Argumentnamen angegeben -- **das
war auch nötig**, da das zweite Argument ansonsten die Auswahl der
...
Spalten, die über die einfache Angabe der Spaltennamen
`character`-Vektor hinausgehen, die wir von der `[·,·]`-Notation kennen.
Zunächst einmal können wir Spaltennamen ohne Anführungszeichen
anzugeben: 
...
Spaltenauswahl zu, der einiges an Schreibarbeit ersparen kann: Wir können
den Doppelpunktoperator verwenden, um mehrere Spalten auszuwählen.
