...
Beachtet, dass die Argumente `yes` und `no` im allgemeinen Fall 
„vektorisiert“ sind, also dieselbe Länge wie das Argument `test` haben 
können. In diesem Fall wird positionsweise entschieden, welche Elemente 
...
Diese Vektorisierung ist oftmals der Grund, warum man die Funktion 
`ifelse` überhaupt verwendet, für aber uns erst mal nicht von Interesse. 
Wir nutzen `ifelse` zunächst nur zur dichotomen Bepunktung von Items 
(durch Abgleich von Antworten mit einem Schlüssel). 
