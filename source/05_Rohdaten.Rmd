...
- `yes`: Die Ausgabe an den Positionen, an denen der `test` `TRUE` ergab
- `no`: Die Ausgabe an den Positionen, an denen der `test` `FALSE` ergab

Das Ergebnis des Vergleichs, der dem Argument `test` übergeben  wird,
ist in diesem Fall dieser folgende logische Vektor:

```{r}
bundesland_answers == bundesland_key
```

An allen Positionen, an denen der Vergleich `TRUE` ergab, steht in der
Ausgabe der Wert 1, also der Wert, der dem Argument `yes` übergeben
wurde. An den anderen Positionen steht 0. Praktisch: Ich muss nicht
angeben, welche falschen Werte alle auftreten können; es reicht aus, im
