...
tatsächlich steckt hier etwas mehr drin als wir bislang behandelt haben.
Wir nehmen zunächst einmal einfach hin, dass man die Umkodierung von
Werten in `data.frames` genauso durchführen kann wie in Vektoren. Wie
wir  schon bei Vektoren kennengelernt haben, sind viele Operationen in
`R` so allgemein, dass sie auf allen Daten eines Objekts stattfinden. 
...
kein gutes Vorgehen, 0 als Kodierung für fehlende Werte zu nehmen. Der
Wert ist nicht ausreichend unterschiedlich von anderen „legalen Werten“.
Der Wert `-99` wäre ein besserer Wert gewesen, um fehlende Werte zu
kodieren. 
...
`nrow(temp)` zurück, der pro Zeile kodiert, ob irgendwie ein fehlender
...
ausgewählt haben. Dabei ist unser nächstes Ziel für alle 40 Items eine
dichotome Bepunktung durchzuführen. Wir wissen bereits, wie das
funktioniert, nämlich indem wir mit `ifelse` die Antworten auf jedes
Item mit dem Schlüssel abgleichen. Der Schlüssel für den das NPI kodiert
für jedes der 40 Items den Wert, der für Narzissmus steht. Dies wäre wie
folgt möglich:
...
Eine allgemein nützlich Funktion zum Betrachten von `data.frames` ist
