...
### Vektorisierung der Funktion `ifelse()`
...
```{r}
item1_inv <- ifelse(big5$item1 == 1, 5, NA)
item1_inv <- ifelse(big5$item1 == 2, 4, item1_inv)
item1_inv <- ifelse(big5$item1 == 3, 3, item1_inv)
item1_inv <- ifelse(big5$item1 == 4, 2, item1_inv)
item1_inv <- ifelse(big5$item1 == 5, 1, item1_inv)
```
...
```{r}
cor(item1_inv, big5$item1)
```

Bei mehrfacher Verwendung von `ifelse()` zur Umkodierung verschiedener 
Werte in einem Vektor nutzen wir `NA` als Platzhalter für die Elemente, 
die bislang noch nicht umkodiert wurden. Wenn keine mathematische 
Formulierung für eine Umkodierung existiert, ist es durchaus gängig, 
dass Umkodierungen genau in dieser Form per mehrfacher Verwendung von 
`ifelse()` umsetzt werden. 
