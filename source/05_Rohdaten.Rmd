...
Bei der Dateneingabe des Tests würde vermutlich zwischen 1 und 4
kodiert,  welche der vier Optionen ausgewählt wurde. Ob ein Kreuz bei
(1), (2), (3) oder (4) gesetzt wird, ist für die Auswertung nicht
unbedingt  von Belang. Erst einmal ist relevant, ob die Frage richtig
...
`npi$Q1` enthält nicht nur die Werte 1 und 2, sondern auch Null-Werte,
wie wir mit dem Befehl `table(npi$Q1)` prüfen können:[^plausicheck]
...
Als nächstes wollen wir alle Fälle ausschließen, bei denen mindestens
...
ist. Der erste Schritt dabei wird sein, alle fehlenden Werte 
in `NA` umzuwandeln. Nur `NA` interpretiert 
`R` tatsächlich als fehlend; Konventionen, die Werte wie -99 oder Null 
als fehlend klassifizieren, kennt `R` nicht. Dabei werden uns folgende 
Funktionen helfen:

- `is.na`: Gibt für einen Vektor aus, welche Elemente `NA` sind
- `complete.cases`: Gibt für einen `data.frame` aus, welche Zeilen 
  **keine** fehlenden Werte enthalten
- `na.omit`: Gibt von einem `data.frame` nur die Zeilen ohne fehlende 
  Werte aus

Wir betrachten im Folgenden zwei Möglichkeiten alle Fälle mit fehlenden 
Antworten auszuschließen.

### Ausschluss von Fällen -- Methode 1

Um fehlende Fälle auszuschließen, speichern wir zunächst die Antworten
auf die 40 Items und die Fallnummer in einem neuen `data.frame` ab.
Anhand dieses `data.frames` werden wir die Fallausschlüsse durchführen:

```{r}
 items <- npi[, c("casenum", paste0("Q", 1:40))]
```

Wir können jetzt Null-Werte in `NA` umkodieren, indem wir ein Vorgehen
...
nachdem, ob der fehlende Wert `TRUE`  oder `FALSE` wäre, würde sich ja
eine unterschiedliche Ausgabe ergeben. `TRUE | NA` hingegen ergibt
`TRUE`, da auf jeden Fall schon eine Bedingung erfüllt ist und insgesamt
die ODER-Bedingung `TRUE` ergibt.

Um einen Überblick über die Verteilung der fehlenden Fälle zu
erhalten, bietet sich eine erneute Anwendung der Funktion `summary` an.
Diese gibt nämlich direkt für jede Spalte eines `data.frames` die Zahl
der fehlenden Fälle an. Folgende Information gibt es zum ersten Item:

&nbsp;

```{r, echo = FALSE}
kable(summary(items)[,2], col.names = "Q1")
```
...
irgendwo_na <- is.na(rowSums(items))
```
...
Wie wir sehen, haben wir `r nrow(npi) - nrow(items)` Fälle
...
`items` abgespeichert haben. Vergleichen wir:
...
Tabelle `items` zusammenführen.

```{r}
npi_clean <- merge(npi, items)
```
...
## Arbeiten mit dem bereinigten Datensatz

Mit dem Datensatz `npi_clean` können wir nun psychometrische
Berechnungen durchführen können, da wir nur vollständige Datensätze
ausgewählt haben.[^nochmehrplausichecks] Dabei ist unser nächstes Ziel
für alle 40 Items eine dichotome Bepunktung durchzuführen. Wir wissen
bereits, wie das funktioniert, nämlich indem wir mit `ifelse` die
Antworten auf jedes Item mit dem Schlüssel abgleichen. Der Schlüssel für
den das NPI kodiert für jedes der 40 Items den Wert, der für Narzissmus
steht. Dies wäre wie folgt möglich:
