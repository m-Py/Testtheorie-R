...
`[·,·]`-Zugriffe. Dieser Abschnitt sollte also spätestens jetzt gelesen
werden.
...
durchzuführen. Beispielsweise können wir Umkodierungen von Items, die
wir aus Kapitel 4 kennen, nicht nur für ein einzelnes Item, sondern
"gleichzeitig" für alle Items eines Tests durchführen. Wie auch eigene
Funktionen helfen uns Schleifen bei der Automatisierung unserer
Arbeit. Sie helfen uns, `R` als Programmiersprache zu nutzen.

Im Allgemeinen und in `R` im Speziellen gibt es mehrere schleifen-artige
...
durch. Wie wir festlegen, wie oft der Code durchgeführt wird und wie wir
es schaffen, dass nicht einfach jedes Mal dasselbe passiert, lernen wir
im Folgenden. Dies ist die Syntax einer `for`-Schleife:
...
Das sieht erst einmal etwas beunruhigend aus. Gehen wir die 
einzelnen Bestandteile der Schleife einmal durch; danach schauen uns 
eine `for`-Schleife in Aktion an. 
...
`(Schleifenvariable in vector)`. Der Ausdruck `vector` kann dabei ein
beliebiger Vektor sein, etwa einer der folgenden:
...
Wie oft der Körper der Schleife -- eingeschlossen in den geschwungenen
Klammern `{·}` -- ausgeführt wird, hängt von der Länge des Vektors ab,
der ihr übergeben wird. In den vorliegenden drei Beispielen würde die
Schleife viermal, dreimal, oder zehnmal durchgeführt werden.

Um zu verstehen, warum es überhaupt Sinn macht, denselben Code-Block
mehrfach durchzuführen -- das klingt ja erst mal etwas seltsam --
betrachten wir den Ausdruck `Schleifenvariable`. Hier offenbart sich die
Magie der `for`-Schleife: **Der `Schleifenvariable` wird in jedem neuen
Schleifendurchlauf sukzessive das nächste Element von `vector`
...
jeden Durchlauf der Schleife. Betrachten wir folgendes
...
einen Punkt vorgeben, wenn die narzisstische Aussage gewählt wurde.
...
hat. Dann folgen `Q2`, `Q3`, usw. Als Namen von Schleifenvariablen
werden häufig kurze Namen wie `i` oder `j` verwendet, insbesondere wenn
es sich um *Indexvariablen* handelt, sie also eine numerische Sequenz der
Form `1:n` durchlaufen.
...
einer Spalte mit dem Namen `colname` suchen, aber `colname` ist ja die
Variable, die den eigentlichen Spaltennamen (etwa `Q23`) enthält.
...
Probanden beim i'ten Item die narzisstische Aussage ausgewählt
haben. Eine 1 wird vergeben, wenn das der Fall war, andernfalls eine
Null. Ich speichere diesen numerischen Vektor aus Einsen und Nullen in
der Variablen `narcissistic_response` zwischen. Beachtet, dass diese
Variable in jedem Durchlauf der Schleife neu geschrieben wird (dasselbe
gilt für die Variablen `colname`, `ith_item` und `new_colname`) .

Der vierte Befehl im Körper der Schleife generiert neuen Spaltennamen,
um die 1/0-Vektoren, die die Auswahl narzisstischer Aussagen kodieren,
als Spalten meines `data.frames`, abzuspeichern. So stelle ich sicher,
dass ich auch später darauf zugreifen kann, etwa um Summenwerte über
alle Items oder Item-Trennschärfen zu bestimmen. Die Funktion `paste0`
generiert die neuen Spaltennamen in der Form `codedQ1`, `codedQ2`, usw.

Der fünfte Befehl im Körper der Schleife fügt mit der `[[·]]`-Notation
die umkodierten Narzissmus-Werte als Spalte an `npi_clean` zu.

Beachtet, dass ich die `for`-Schleife auch mit weniger
"zwischenspeichern" hätte umsetzen können; folgender Code würde in
...
Variablen abgespeichert, um den Code besser verständlich zu machen. Aus
meiner Sicht ist das Zwischenspeichern in Variablen gut geeignet, um zu
kommunizieren, was Code macht -- insbesondere, wenn die Variablen gut
benannt sind. Außerdem war es so leichter, die einzelnen Arbeitsschritte
der Schleife zu erläutern.
...
angemessene Form umkodiert habe, kann ich mit meiner Analyse
starten. Ein wichtiger Teil einer Item-Analyse ist die Berechnung von
Item-Trennschärfen. Dieser Abschnitt behandelt die Frage, wie wir
mithilfe einer `for`-Schleife korrigierte Item-Trennschärfen für alle 40
Items des NPI berechnen können. Wir nutzen diesen Code:
...
aus, die ich in den Durchläufen der Schleife berechne; um nicht eine
ganze Seite mit Trennschärfen vollzumachen, habe ich hier aber nur drei
davon angezeigt. Um ein Objekt während des Laufs einer Schleife in der
Konsole auszugeben, muss man `print` explizit auf das auszugebende
Objekt aufrufen; das Objekt ohne `print` anzusteuern würde in keiner
Reaktion resultieren.[^`r incref()`]
...
Spalte mit dem Namen abgespeichert ist, der sich gerade in der Variablen
`name` befindet. Betrachtet den Code genau: Mithilfe der `[·,·]`-Notation
werden genau die 39 anderen Spalten ausgewählt (siehe [Kapitel
3.5](#fortgeschritten)). Dafür wird hinter dem Komma der
`[·,·]`-Notation ein logischer Vektor der Länge 40 übergeben, der nur an
einer Stelle `FALSE` enthält und sonst `TRUE`. Dieser Vektor wird mit
dem Befehl `name != colnames(items)` erstellt.
...
wir zwei Fälle: Einmal adressieren adressieren wir Vektorelemente per
Name und einmal per Index (siehe [Kapitel 3.5](#nameindex)).
...
Spalten des `data.frames` `items`, den ich verwendet habe zur Berechnung
der Trennschärfen. Dass ich ausgerechnet diese Namen vergeben habe, hat
zur Folge, dass ich recht einfach den obigen Code zur Berechnung der
Trennschärfen umwandeln kann, um die Trennschärfen auch noch
abzuspeichern. Dies ist der leicht angepasste Code:
