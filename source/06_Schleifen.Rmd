...
erlauben uns, eine Aufgabe mehrfach durchzuführen, ohne dass wir dafür
Code mehrfach schreiben müssen. Beispielsweise müssen wir Umkodierungen
von Items, wie wir sie in Kapitel 4 durchgeführt haben, nicht für jedes
einzelne Item neu eingeben -- d.h. fehleranfällig copy-pasten --,
sondern wir können sie auf einmal für alle Items durchführen. Wie auch
eigene Funktionen helfen uns Schleifen bei der Automatisierung unserer
...
zugegriffen wird. Im Folgenden lernen wir, wie wir festlegen, wie oft
ein Code-Block durchgeführt wird und wie wir es schaffen, dass nicht in
jedem Durchgang einfach dasselbe passiert. Dies ist die Syntax einer
`for`-Schleife:
...
Vektor. Von der Länge dieses Vektors hängt ab, wie oft der Körper der
Schleife -- eingeschlossen in den geschwungenen Klammern `{·}` --
ausgeführt wird. Wir könnten der `for`-Schleife beispielsweise einen der
folgenden Vektoren übergeben:
...
zusätzlich den Ausdruck `Schleifenvariable`. Hier offenbart sich die
...
zugeordnet.** `Schleifenvariable` ist also eine Variable, auf die wir im
Körper der Schleife zugreifen können. Ihr Inhalt ändert sich in jedem
Durchlauf der Schleife. Betrachten wir folgendes Spielzeug-Beispiel, das
das Konzept der `for`-Schleife verdeutlicht:
...
- Im Körper der Schleife können wir auf die Schleifenvariable zugreifen,
  um in jedem Durchlauf anderen Berechnungen durchzuführen.
...
automatisieren, das heißt für alle 40 Items des NPI durchführen. Dafür
benötige ich für jedes Item des NPI den Schlüssel, den ich dem Codebuch
entnehmen kann. Wir übertragen die 40 Schlüssel manuell in einen Vektor:
...
schon eingelesen wurden und die Datenbereinigung aus Kapitel 4
...
Der vierte Befehl im Körper der Schleife generiert mit einem Aufruf der
Funktion `paste0` in jedem Durchlauf einen neuen Spaltennamen. Die neuen
Spalten haben Namen der Form `codedQ1`, `codedQ2` und so weiter.

Der fünfte Befehl im Körper der Schleife fügt mit der `[[·]]`-Notation
die umkodierten Narzissmus-Werte als Spalte an `npi_clean` hinzu. So
stelle ich sicher, dass ich auch später darauf zugreifen kann, etwa um
Summenwerte über alle Items oder Item-Trennschärfen zu bestimmen.
...
Item-Trennschärfen (siehe [Kapitel 4](#trennschaerfe)). Dieser Abschnitt
behandelt die Frage, wie wir mithilfe einer `for`-Schleife korrigierte
Item-Trennschärfen für alle 40 Items des NPI berechnen können. Wir
nutzen zunächst diesen Code:
...
aus, die ich in den Durchläufen der Schleife berechne; um die Seite nicht
