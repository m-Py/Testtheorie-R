...
`R`-Vektor, den wir der `for`-Schleife übergeben. Wie oft der Körper der
Schleife -- eingeschlossen in den geschwungenen Klammern `{·}` --
ausgeführt wird, hängt von der Länge von `vector` ab; `vector` könnte
beispielsweise einer der folgenden Vektoren sein:
...
verstehen, warum es überhaupt Sinn macht, denselben Code-Block mehrfach
...
Dieser logische Vektor wird mit dem Befehl `name != colnames(items)`
erstellt.
