...
## Maximum insgesamt, nutze c(), um die Springer zusammenzufuegen:
alle_spruenge <- c(us_springer_m, d_springer)
max(alle_spruenge)
min(alle_spruenge)


## Maximum / Minimum per Land:
max(us_springer_m)
max(d_springer)
...
sum(us_springer_m > 2) + sum(d_springer > 2)

## Aequivalent:
sum(c(us_springer_m, d_springer) > 2)
...
disqualifiziert <- c(TRUE, FALSE, FALSE, 
                     FALSE, TRUE, FALSE)

## Setze die Sprunghoehen aller disqualifizierten Deutschen
## auf 0.

d_springer[disqualifiziert] <- 0
