...
## Alternative mit der [.,.]-Notation:
temp <- npi[, names(npi) != "score"]
...
temp[temp == 0] <- NA
summary(temp) # zeigt unter anderem fehlende Werte an
...
npi$Ueberlegenheit <- rowSums(npi[, items])
...
sd(npi$Ueberlegenheit)

## (ii) Einmal getrennt fuer maennliche und weibliche
## Teilnehmer/innen

# maennlich
mean(npi$Ueberlegenheit[npi$gender == 1])
sd(npi$Ueberlegenheit[npi$gender == 1])

# weiblich
mean(npi$Ueberlegenheit[npi$gender == 2])
sd(npi$Ueberlegenheit[npi$gender == 2])

## (iii) Unterscheiden sich Frauen und Maenner
## signifikant im Ueberlegenheitsgefuehl? (Rechnet
## einen t-Test)

t.test(npi$Ueberlegenheit[npi$gender == 1],
       npi$Ueberlegenheit[npi$gender == 2])
