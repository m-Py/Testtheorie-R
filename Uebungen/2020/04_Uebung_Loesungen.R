...
tp[tp$casenum > 1000 & tp$geschlecht == "maennlich", ]
...
spaltennamen <- c("TP1", "TP2", "TP3", "TP4", "TP5", "TP6", "TP7",
                  "TP8", "TP9", "TP10i", "TP11i", "TP12i")

items <- tp[, spaltennamen]

einsbisneun <- paste0("TP", 1:9)
zehnbis12 <- paste0("TP", 10:12, "i")

items <- tp[, c(einsbisneun, zehnbis12)]
...
cor(tp$TP11, tp$TP11i)
cor(tp$TP12, tp$TP12i)
...
sum(rowSums(items) != tp$sum)

tp$mean_score <- rowMeans(items)
...
t.test(maennlich$mean_score, weiblich$mean_score)

t.test(mean_score ~ geschlecht, data = tp)

t.test(
  tp$mean_score[tp$geschlecht == "maennlich"],
  tp$mean_score[tp$geschlecht == "weiblich"]
)
